...
function calculateChildrenWeightSum(node: TreeNode): number {
    return node.children.reduce((sum, child) => sum + child.edge.weight, 0);
}
            ...
            <table className="w-full text-sm table-fixed">
                ...
                <tbody>
                    {ordered.map((node) => {
                        ...
                        const childrenWeightSum = calculateChildrenWeightSum(node);
                        const hasChildren = node.children.length > 0;
                        const isOverweight = childrenWeightSum > 100;
                        ...
                        return (
                            <tr key={edge.edget_id} className="border-t">
                                ...
                                <td className="p-2 align-top text-muted-foreground break-words whitespace-pre-wrap">
                                    {edge.description}
                                </td>
                                <td className="p-2 align-top">
                                    {hasChildren ? (
                                        <span
                                            className={
                                                isOverweight ? "text-red-600 font-medium" : ""
                                            }
                                        >
                                            {edge.weight} ({childrenWeightSum})
                                        </span>
                                    ) : (
                                        edge.weight
                                    )}
